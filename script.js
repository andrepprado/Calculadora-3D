...
function calcular() {
    ...
    const canalSelect = document.getElementById('canalVenda');
    const percTaxaCanal = parseFloat(canalSelect.value) || 0;
    let valorTaxaFixaCanal = 0;
    if(percTaxaCanal > 0 && canalSelect.selectedOptions[0].text.includes("Shopee")) valorTaxaFixaCanal = CONFIG.taxa_fixa_shopee * qtd;
    ...
    salvarConfiguracoes(margemLucro);
}

function format(v) { return v.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' }); }

function salvarConfiguracoes(margem) {
    const filIndex = document.getElementById('filamentoSelect').value;
    localStorage.setItem('cal3d_duolab_final', JSON.stringify({ margem, filIndex }));
}
...
document.getElementById('filamentoSelect').addEventListener('change', atualizarFilamento);
